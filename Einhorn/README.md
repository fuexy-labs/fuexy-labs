...
* **Custom Power-Supply:** Einbau eines **Step-Down-Wandlers** (19V -> 5V / 5A), um die USB-Schiene stabil zu versorgen.
...
1. **BIOS-Force-Aktivierung:** Da kein internes Display verbaut ist, wird das BIOS via Hardware-Programmer (**ASProgrammer**) so modifiziert, dass das Bildsignal sofort auf die externen Ausgänge (VGA/HDMI) gezwungen wird.
2. **Power-Table Tuning:** Feinjustierung der Power-Limits (PL1/PL2) innerhalb der stabilen 38W-Limitierung.
3. **Hardware-Schutz:** Die Beibehaltung des 38W-Limits dient als bewusster Schutz für die VRMs (Spannungswandler).
